...
class Spectral(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, vertices, edges, faces, x, el, ts, corners, minCurvature, maxCurvature, rotationNormal, anisotropy1, anisotropy2, Theta, voronoi, cache=False, rewrite=False):
        ...
        for i in range(batch_size):
            ...
            cached2 = None
            if cache:
                hash_key = tensor_hash(verts, facs, edge)
                if not rewrite:
                    cached = load_from_cache(hash_key)
                    cached2 = load_from_cache2(hash_key)
            ...
            if not baseline:
                if cache and cached is not None:
                        A, L, angles[i][facsOffset[i]:], edge_length[i][facsOffset[i]:], ofD1[i][:, facsOffset[i]:], ofD2[i][:, facsOffset[i]:], ofDR[i][:, facsOffset[i]:], dmax[i][:, facsOffset[i]:], dmin[i][:, facsOffset[i]:], drot[i][:, facsOffset[i]:], gV[i][edgeOffset[i]:], gL[i][edgeOffset[i]:], Aub[i][vertOffset[i]:] = cached
                else:
                    ...
                    if cache:
                        save_to_cache(hash_key, (A, L, angles[i][facsOffset[i]:], edge_length[i][facsOffset[i]:], ofD1[i][:, facsOffset[i]:], ofD2[i][:, facsOffset[i]:], ofDR[i][:, facsOffset[i]:], dmax[i][:, facsOffset[i]:], dmin[i][:, facsOffset[i]:], drot[i][:, facsOffset[i]:], gV[i][edgeOffset[i]:], gL[i][edgeOffset[i]:], Aub[i][vertOffset[i]:]))
            else:
            ...
            if cache and cached2 is not None:
                    (eigvalue, eigvector) = cached2
            else:
                try:
                    (eigvalue, eigvector) = lg.eigsh(L, frequency+foff, sparse.diags(A), sigma=-0.01)
                    if cache:
                        save_to_cache2(hash_key, (eigvalue, eigvector))
                except Exception as e:
                    print("bypassed")
                    A, L, angles[i][facsOffset[i]:], edge_length[i][facsOffset[i]:] = get_cotan_laplacian_igl(verts, facs,
                                                                                                              ks * 0, edge)
                    L = L + sparse.identity(L.shape[0]) * eps
            ...
            eigvalue1 = torch.abs(torch.tensor(eigvalue[foff:], requires_grad=False))
            eigvalue1 -= eps
            eigvalues[i], idx = torch.sort(eigvalue1)
            eigvectors[i] = torch.tensor(np.pad(eigvector[:, foff:], [(vertex_count - verts.shape[0], 0), (0, 0)]), requires_grad=False).index_select(-1, idx)  #
    ...
    def backward(ctx, hks):
        ...
        for sample in torch.arange(batch_size, dtype=torch.int32, device=device):
            ...
            k1 = torch.diag(ITris[tsl][:, k1_index])
            k2 = torch.diag(ITris[tsr][:, k2_index])
            ...
            if Riemann:
                ...
                indicesLx2 = torch.tensor((i[0], j[0], i[0], j[0], k2[0], k1[0], i[0], k2[0], j[0], k2[0], i[0], k1[0], j[0], k1[0]), device=device)
                indicesLy2 = torch.tensor((i[0], j[0], j[0], i[0], k2[0], k1[0], k2[0], i[0], k2[0], j[0], k1[0], i[0], k1[0], j[0]), device=device)
                ...
                masterIndex = torch.cat((indicesLx2 * 4 + indicesLy2, torch.tensor([11, 14], device=device)))[None, None, :].repeat(M, frequency, 1)
            ...
            if ALBO:
                ...
                perc99 = torch.quantile(dFaceValueI, 0.99, keepdim=True)
                dFaceValueI = torch.clip(dFaceValueI, min=perc01, max=perc99)
                dFaceValue2[sample, offsetF:] = dFaceValueI  # /(torch.linalg.vector_norm(dFaceValueI)*16)
            ...
            if plot:
                if Voronoi:
                    p = pv.Plotter()
                    p.set_background('gray', top='white')
                    p.enable_anti_aliasing()
                    plotFacesAnimate(VPos.cpu().numpy(), ITris.cpu().numpy(),
                                    (dVertexValue[sample, offset:].cpu().numpy()), edge, p)
                    p.show()

                if Riemann:
                    plotEdges(VPos.cpu().numpy(), ITris.cpu().numpy(), dEdgeValue[sample, offsetE:].cpu().numpy(),
                              edge.cpu().numpy())
                if ALBO:
                    p = pv.Plotter()
                    p.set_background('gray', top='white')
                    p.enable_anti_aliasing()
                    plotFacesAnimate(VPos.cpu().numpy(), ITris.cpu().numpy(),
                                     (dFaceValue1[sample, offsetF:].cpu().numpy()), edge, p)
                    #
                    p.show()
                    ...
                    p.enable_anti_aliasing()
                    plotFacesAnimate(VPos.cpu().numpy(), ITris.cpu().numpy(),
                                     (dFaceValue3[sample, offsetF:].cpu().numpy()), edge, p)
                    #
                    p.show()
